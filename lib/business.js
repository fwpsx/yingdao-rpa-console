...
// 账号归一化：/account/current 的响应可能是 { Account } / { User } / 扁平对象三种形态，
// 统一成小写驼峰。抽成单点，避免"同一契约各写一套"（此前有三处各写一遍）。
// 缺少 userId 时返回 null —— 调用方（exportTriggers / matchTriggers）已对 null 做了守卫。
function normAccount(data) {
  if (!data) return null;
  const a = data.Account || data.User || data;
...
async function checkAccountSwitch() {
  try {
    if (await restReady()) {
      ...
      if (r.ok && r.data) {
        const acc = normAccount(r.data);
        if (acc) {
          if (lastKnownUserId !== null && lastKnownUserId !== acc.userId) {
            clearCache(); // 账号已变化，清空全部数据缓存
          }
          lastKnownUserId = acc.userId;
        }
      }
    } else {
...
// 只导出跨模块真正用到的符号，其余保持内部可见：
// 无用的导出会让模块边界虚化，也让人误以为外部有依赖（这 8 个此前外部引用为 0）。
module.exports = {
  normTask, normAccount,
  fetchAllApps, fetchAllGroups,
  exportTriggers, matchTriggers, importTriggers,
  exportGroups, matchGroups, importGroups,
  checkAccountSwitch,
};
